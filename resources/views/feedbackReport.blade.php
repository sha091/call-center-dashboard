                        ...
                        <div class="row">
                            <div class="col-lg-6 col-sm-6 col-md-6">
                                <ol class="breadcrumb bg-transparent mb-0 pb-0 pt-1 px-0 me-sm-6 me-5">
                                    ...
                                    <li class="breadcrumb-item text-sm text-dark active" aria-current="page">Feedback Report</li>
                                </ol>
                            </div>
                        </div>
                    </div>
                                ...
                                <div class="modal fade" id="myModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                    <div class="modal-dialog" role="document">
                                        <div class="modal-content">
                                            ...
                                            <form action="{{ route(auth()->user()->designation == 'Supervisor' ? 'feedback.report' : 'agent.missed.call.reports') }}" method="GET">
                                                @csrf
                                                ...
                                                <div class="modal-footer">
                                                    <a href="javascript:void(0);" type="button" class="btn btn-secondary" data-bs-dismiss="modal" onclick="window.location='{{ route(auth()->user()->designation == 'Supervisor' ? 'feedback.report' : 'agent.missed.call.reports') }}'">Clear</a>
                            ...
                            <tbody>
                                @forelse ($cdrs as $key => $value)
                                    <tr>
                                        ...
                                        <td>{{ $value->is_feedback == 1 ? "Satisfactory" : "Unsatisfactory" }}</td>
                                        <td>{{ date('Y-m-d h:i A', strtotime($value->call_datetime)) }}</td>
                                    </tr>
                                    @empty
                                        <tr>
...
<script>
    $(document).ready(function() {
        $('#exportBtn').on('click', function() {
            ...
            var routeCallCenterStatus = "{{ route(auth()->user()->designation == 'Supervisor' ? 'export.feedback.reports' : 'export.feedback.reports') }}";
