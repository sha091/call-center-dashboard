                    ...
                    <div class="card-header p-3 pt-2 bg-gradient bg-white">
                        <div class="row my-1" >
                            ...
                            <div class="col-md-6 col-lg-6 col-sm-6 p-3">
                                <!-- Button trigger modal -->
                                <a href="{{ route('settings.working.hours') }}" class="btn btn-danger badge float-end "><i class="material-icons opacity-10">arrow_back</i></a>
                            </div>
                        </div>
                        <div class="container  py-4">
                            <form action="{{ route('settings.update.WorkingHours') }}" method="POST">
                                @csrf
                                <div class="row my-1">
                                    ...
                                    <div class="col-2 offset-1">
                                        <span class="text-danger">Select Option</span>
                                    </div>
                                </div>
                                @foreach ($hours as $value)
                                    <div class="row my-4">
                                        ...
                                        <div class="col-2 offset-1">
                                            <input type="text" class="form-control bs-timepicker" value="{{ $value->start_time }}"  name="start_time[{{ $value->id }}]"/>
                                        </div>
                                        <div class="col-2 offset-1">
                                            <input type="text" class="form-control bs-timepicker" value="{{ $value->end_time }}"  name="end_time[{{ $value->id }}]" />
                                        </div>
                                        <div class="col-2 offset-1">
                                            <select class="form-select"  name="agent_id[]">
                                                <option value="ivr" {{ $value->option_off_time == 'ivr' ? 'selected' : '' }}>Offtime Prompt</option>
                                                <option value="voicemail" {{ $value->option_off_time == 'voicemail' ? 'selected' : '' }} >Voicemail</option>
