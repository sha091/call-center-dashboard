                        ...
                        <div class="row">
                            <div class="col-lg-6 col-sm-6 col-md-6">
                                <ol class="breadcrumb bg-transparent mb-0 pb-0 pt-1 px-0 me-sm-6 me-5">
                                    ...
                                    <li class="breadcrumb-item text-sm text-dark active" aria-current="page">Missed Calls Report</li>
                                </ol>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="row my-1">
                                ...
                                <div class="modal fade" id="myModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                    <div class="modal-dialog" role="document">
                                        <div class="modal-content">
                                            ...
                                            <form action="{{ route(auth()->user()->designation == 'Supervisor' ? 'missed.call.reports' : 'agent.missed.call.reports') }}" method="GET">
                                                @csrf
							...
							@endif
                                                ...
                                                <div class="modal-footer">
                                                    <a href="javascript:void(0);" type="button" class="btn btn-secondary" data-bs-dismiss="modal" onclick="window.location='{{ route(auth()->user()->designation == 'Supervisor' ? 'missed.call.reports' : 'agent.missed.call.reports') }}'">Clear</a>
                        ...
                        <table id="Agent-Statistaics-Summary" class="table table-success table-striped table-bordered  dt-responsive nowrap text-center" style="width:100%">
                            ...
                            <tbody>
                                @forelse ($cdrs as $key => $value)
                                    <tr>
                                        <td>{{ $key+1 }}</td>
                                        <td>{{ isset($value->full_name) ? $value->full_name : "IVR" }}</td>
                                        <td>{{ $value->caller_id }}</td>
                                        <td>{{ $value->call_date }}</td>
                                        <td>{{ date('h:i:s A', strtotime($value->TIME)) }}</td>
                                        <td>{{ differenceInDays($value->staff_end_datetime,$value->call_datetime) }}</td>                                        
                                        <td>{{ $value->call_id }}</td>
                                    </tr>
                                    @empty
                                        <tr>
...
<script>
    $(document).ready(function() {
        $('#exportBtn').on('click', function() {
            ...
            let agent_id = "{{ isset($_GET['agent_id']) ? $_GET['agent_id'] : false }}";
            var routeCallCenterStatus = "{{ route(auth()->user()->designation == 'Supervisor' ? 'export.missed.call.reports' : 'agent.export.missed.call.reports') }}";
